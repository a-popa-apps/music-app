...
6. ~~Say the "one tool, not two" thing out loud.~~ **Done (2026-09-15).** Added a "One Tool, Start To Finish" feature card and sharpened the Pro plan's pricing subtitle to name the detection+cleanup+set-ordering pipeline directly instead of just listing features -- doesn't name SetFlow or any competitor, states the capability positively.
...
8. ~~Add MusicBrainz + iTunes as extra genre-lookup fallbacks.~~ **Done (2026-09-15), and went further** — shipped MusicBrainz, iTunes, Deezer, TheAudioDB, and Last.fm (5 new sources, not just 2), plus cover art embedding as a side effect of the same lookups. See the catalog-source table above.
9. ~~Don't lean on "competitively priced" without qualification.~~ **Done (2026-09-15).** Asked directly whether to lower prices to compete -- decided against it: neither GreenGo (matches price, adds stem separation) nor One Tagger (free) would be beaten on price alone, so a cut would cost margin without closing either gap. Landing copy now leads with capability breadth instead (see item 6).
