...
from app import ai_budget, ai_client, preview_cache, process_audio, rate_limit
...
def _isolate_preview_cache(tmp_path, monkeypatch):
    """preview_cache.CACHE_DIR is a real (if capped) local-disk location --
    each test gets its own empty directory so a preview generated by one
    test can't produce a false cache hit in another, and nothing here ever
    touches whatever real cache directory a live server is using."""
    monkeypatch.setattr(preview_cache, "CACHE_DIR", str(tmp_path / "preview_cache"))
