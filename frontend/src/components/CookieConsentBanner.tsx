...
import { ANALYTICS_ENABLED, getStoredConsent, setAnalyticsConsent } from "../utils/analytics"

export function CookieConsentBanner() {
  const [visible, setVisible] = useState(false)

  // A granted consent from a previous visit is already re-applied at
  // analytics.ts's module load, before this or any other component mounts
  // -- this effect only needs to decide whether the banner itself shows.
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 flex flex-col items-center gap-3 border-t border-outline-variant bg-inverse-surface px-4 py-4 text-inverse-on-surface sm:flex-row sm:justify-between sm:gap-6 sm:px-8">
      <p className="text-body-sm">
        We use Google Analytics to understand how CratePrep is used, and the Meta Pixel to
        measure how our ads perform. We don't sell your data. See our{" "}
